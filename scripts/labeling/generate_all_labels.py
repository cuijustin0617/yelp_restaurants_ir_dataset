...
Respond with ONLY the numbered True/False values, no other text."""
    ...
    while True:
        try:
            ...
            print(f"Attempt {attempt} failed: {str(e)}. Retrying in 5 seconds...")
            time.sleep(5)
            attempt += 1
...
def main():
    ...
    queries_path = 'queries/queries_new_100_2.txt'  ################TO CHANGE
    
    # Output directory for storing relevance judgement CSV files, one csv per restaurant
    output_folder = Path('Philadelphia/judgements_new_2')  ################TO CHANGE

    # Process each restaurant document
    docs = list(docs_dir.glob('*.txt'))
    with tqdm(docs, desc="Processing restaurants", unit="doc", 
        ...
        for doc_path in pbar:
            ...
            if not output_path.exists():
                ...
                tqdm.write(f"Skipping {doc_path} as it already has a judgement.")
